...
Tests verify that explicit migration installs these named invariants without destructively dropping or altering shared test-database objects. The SHA-256 predicate is additionally tested behaviorally in a uniquely named temporary schema: a 64-character lowercase hexadecimal value is accepted; short, uppercase, and non-hexadecimal values fail the CHECK constraint; an overlength value is rejected by PostgreSQL's `char(64)` type boundary before the CHECK is evaluated. The temporary schema is dropped in `finally`, so this negative proof does not alter `public.legal_documents` or depend on test ordering. A separate non-destructive test inserts only invalid values into the migrated `public.legal_documents` table and requires the installed named CHECK to reject them, leaving no committed rows.
