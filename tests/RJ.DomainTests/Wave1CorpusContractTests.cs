...
public sealed class Wave1CorpusContractTests
{
    ...
    public void Rjudi_m1_gate_is_local_core_only_and_bound_to_current_artifacts()
    {
        ...
        Assert.Contains("brazilian_document_masker_ascii_digits_only", requiredEvidence);
        Assert.Contains("legal_case_collections_reject_null_items", requiredEvidence);
        ...
        Assert.Contains("process_provenance_observed_instant_required", requiredEvidence);
        Assert.Contains("process_source_observed_instant_required", requiredEvidence);
        Assert.Contains("attachment_content_observed_instant_required", requiredEvidence);
        Assert.Contains("attachment_content_store_rejects_null_items", requiredEvidence);
        Assert.Contains("attachment_content_chunks_validate_evidence_values", requiredEvidence);
        Assert.Contains("generation_citations_validate_hash_and_position", requiredEvidence);
        Assert.Contains("legal_evidence_hits_validate_citation_values", requiredEvidence);
        Assert.Contains("generation_context_items_validate_citation_values", requiredEvidence);
        Assert.Contains("generation_context_validates_case_budget_and_items", requiredEvidence);
        ...
        Assert.Contains("process_validator_rejects_unicode_digit_foreign_cnj", requiredEvidence);
        Assert.Contains("process_validator_rejects_unicode_digit_pii", requiredEvidence);
        ...
        Assert.Contains("missing_attachment_content_observed_instant_sanitized", requiredEvidence);
        ...
        Assert.Contains("negative_process_source_amount_sanitized", requiredEvidence);
        Assert.Contains("invalid_process_source_secrecy_level_type_sanitized", requiredEvidence);
        Assert.Contains("negative_process_source_secrecy_level_sanitized", requiredEvidence);
